...
class CreateBadgesScreen extends StatefulWidget {
  final bool isEdit;
  final String id;

  const CreateBadgesScreen({super.key, required this.isEdit, required this.id});

  @override
  State<CreateBadgesScreen> createState() => _CreateBadgesScreenState();
}

class _CreateBadgesScreenState extends State<CreateBadgesScreen> {
  final formKey = GlobalKey<FormState>();
  late ApiResponse apiResponse;
  late BadgeDetailsModel badgeDetails;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(
            widget.isEdit ? "Edit badge" : "Create new badge",
                        ...
                        child: Form(
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  // Event name
                                  _buildEventTextFieldLabel("Badge name"),
                                  CustomTextField(
                                    controller: badgeNameCtrl,
                                    hint: "Enter a Badge name",
                                    isReadOnly: false,
  ...
  Widget _buildEventTextFieldLabel(String labelText) {
    return Container(
      ...
      child: Text(
        labelText,
        style: TextStyle(color: Colors.black, fontFamily: 'Lato', fontSize: 15),
      ),
    );
  }
  ...
  Future<void> createBadgeDetails() async{
    ...
    try {
      final response = await DioClient().request(
        path: ApiEndPoints.createBadgeEndPoint,
        payload: formData,
  ...
  Future<void> updateBadgeDetails() async {
    if (!formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill all fields.')),
      );
    ...
    try {
      final response = await DioClient().request(
        path: ApiEndPoints.updateBadgeEndPoint+badgeDetails.id,
        payload: formData,
  ...
  void setData() {
   badgeNameCtrl.text = badgeDetails.bedgeName;
    setState(() {});
  }
