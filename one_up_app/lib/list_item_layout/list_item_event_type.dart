...
class ListItemEventType extends StatefulWidget {
  final EventTypeDetailsModel eventTypeDetails;
  final Function() onDelete;
  const ListItemEventType({super.key, required this.eventTypeDetails, required this.onDelete});

  @override
  State<ListItemEventType> createState() => _ListItemEventTypeState();
}

class _ListItemEventTypeState extends State<ListItemEventType> {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              ...
              offset: const Offset(0, 2), // X and Y offset of the shadow
            ),
          ],
      ...
      child: Row(
        ...
        children: [
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      Expanded(child: Text(
                        widget.eventTypeDetails.eventTypeName,
          ...
          ElevatedButton(
            onPressed: () => {
              deleteEvent(widget.eventTypeDetails.id)
            },
          ...
          ElevatedButton(
            onPressed: () => {
              Navigator.push(context,
                  MaterialPageRoute(builder: (context)=> CreateEventTypeScreen(isEdit: true,id: widget.eventTypeDetails.id,))
              )
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.white,
  ...
  Future<void> deleteEvent(String id) async{
    try {
      final response = await DioClient().request(
        path: ApiEndPoints.deleteEventTypeEndPoint+id,
