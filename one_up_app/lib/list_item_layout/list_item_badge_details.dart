...
class ListItemBadgeDetails extends StatefulWidget {
  final BadgeDetailsModel badgeDetails;
  final Function() onDelete;
  const ListItemBadgeDetails({super.key, required this.badgeDetails, required this.onDelete});


  @override
  State<ListItemBadgeDetails> createState() => _ListItemBadgeDetailsState();
}

class _ListItemBadgeDetailsState extends State<ListItemBadgeDetails> {
  late ApiResponse apiResponse;
  Future<void> deleteBadge(String id) async {
    try {
      final response = await DioClient().request(
        path: ApiEndPoints.deleteBadgeEndPoint + id,
          ...
          ElevatedButton(
            onPressed: () => {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => CreateBadgesScreen(
                        isEdit: true,
                        id: widget.badgeDetails.id,
                      )))
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.white,
